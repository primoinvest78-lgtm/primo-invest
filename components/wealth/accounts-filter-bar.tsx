...
export function AccountsFilterBar({
  ...
  const institutions = useMemo(() => uniqueBy(accounts, (a) => a.institutionName), [accounts]);
  const accountTypes = useMemo(() => uniqueBy(accounts, (a) => a.accountType), [accounts]);
  const clients = useMemo(() => {
    const map = new Map<string, string>();
    for (const a of accounts) map.set(a.clientId ?? "none", a.clientName ?? "Sem cliente vinculado");
    return Array.from(map.entries()).sort((a, b) => a[1].localeCompare(b[1]));
  }, [accounts]);

  function set<K extends keyof AccountFilters>(key: K, value: AccountFilters[K]) {
    ...
    <div className="card-premium flex flex-wrap items-end gap-2.5 rounded-2xl p-4">
      <div className="flex flex-col gap-1">
        ...
        <div className="relative">
          ...
          <Input
            placeholder="Conta, instituição ou cliente..."
      ...
      <div className="flex flex-col gap-1">
        ...
        <Select value={filters.accountType} onValueChange={(v) => set("accountType", v ?? "all")}>
      ...
      {hasActiveAccountFilters(filters) ? (
        <Button
          ...
          size="sm"
          variant="ghost"
          onClick={() => onChange(DEFAULT_ACCOUNT_FILTERS)}
