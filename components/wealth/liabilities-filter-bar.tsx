...
export function LiabilitiesFilterBar({
  liabilities,
  filters,
  onChange,
}: {
  ...
  onChange: (filters: LiabilityFilters) => void;
}) {
  const liabilityTypes = useMemo(() => uniqueBy(liabilities, (l) => l.liabilityType), [liabilities]);
  const clients = useMemo(() => {
    const map = new Map<string, string>();
    for (const l of liabilities) map.set(l.clientId ?? "none", l.clientName ?? "Sem cliente vinculado");
    return Array.from(map.entries()).sort((a, b) => a[1].localeCompare(b[1]));
  }, [liabilities]);

  function set<K extends keyof LiabilityFilters>(key: K, value: LiabilityFilters[K]) {
    ...
    <div className="card-premium flex flex-wrap items-end gap-2.5 rounded-2xl p-4">
      <div className="flex flex-col gap-1">
        ...
        <div className="relative">
          ...
          <Input
            placeholder="Nome, categoria ou cliente..."
      ...
      <div className="flex flex-col gap-1">
        ...
        <Select value={filters.liabilityType} onValueChange={(v) => set("liabilityType", v ?? "all")}>
          <SelectTrigger className="w-[170px]" size="sm">
            <SelectValue placeholder="Todas" />
          </SelectTrigger>
          <SelectContent>
            <SelectItem value="all">Todas</SelectItem>
            {liabilityTypes.map((value) => (
      ...
      {hasActiveLiabilityFilters(filters) ? (
        <Button
          type="button"
          size="sm"
          variant="ghost"
          onClick={() => onChange(DEFAULT_LIABILITY_FILTERS)}
