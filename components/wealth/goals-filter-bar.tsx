"use client";

import { Search, X } from "lucide-react";
import { useMemo } from "react";

import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import {
  Select,
  SelectContent,
  SelectItem,
  SelectTrigger,
  SelectValue,
} from "@/components/ui/select";
import type { GoalDetail } from "@/lib/data/wealth";
import {
  DEFAULT_GOAL_FILTERS,
  GOAL_STATUS_LABEL,
  hasActiveGoalFilters,
  type GoalFilters,
  type GoalStatus,
} from "@/lib/utils/goal-helpers";

function uniqueBy<T>(items: T[], keyFn: (item: T) => string | null): string[] {
  const set = new Set<string>();
  for (const item of items) {
    const key = keyFn(item);
    if (key) set.add(key);
  }
  return Array.from(set).sort();
}

export function GoalsFilterBar({
  goals,
  filters,
  onChange,
}: {
  goals: GoalDetail[];
  filters: GoalFilters;
  onChange: (filters: GoalFilters) => void;
}) {
  const goalTypes = useMemo(() => uniqueBy(goals, (g) => g.goalType), [goals]);
  const clients = useMemo(() => {
    const map = new Map<string, string>();
    for (const g of goals) map.set(g.clientId ?? "none", g.clientName ?? "Sem cliente vinculado");
    return Array.from(map.entries()).sort((a, b) => a[1].localeCompare(b[1]));
  }, [goals]);

  function set<K extends keyof GoalFilters>(key: K, value: GoalFilters[K]) {
    onChange({ ...filters, [key]: value });
  }

  return (
    <div className="card-premium flex flex-wrap items-end gap-2.5 rounded-2xl p-4">
      <div className="flex flex-col gap-1">
        <label className="text-[10px] font-bold uppercase text-card-beige-muted-foreground">
          Buscar
        </label>
        <div className="relative">
          <Search className="pointer-events-none absolute left-2.5 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-card-beige-muted-foreground" />
          <Input
            placeholder="Nome, categoria ou cliente..."
            value={filters.search}
            onChange={(e) => set("search", e.target.value)}
            className="h-7 w-[220px] pl-8 text-sm"
          />
        </div>
      </div>

      <div className="flex flex-col gap-1">
        <label className="text-[10px] font-bold uppercase text-card-beige-muted-foreground">
          Categoria
        </label>
        <Select value={filters.goalType} onValueChange={(v) => set("goalType", v ?? "all")}>
          <SelectTrigger className="w-[170px]" size="sm">
            <SelectValue placeholder="Todas" />
          </SelectTrigger>
          <SelectContent>
            <SelectItem value="all">Todas</SelectItem>
            {goalTypes.map((value) => (
              <SelectItem key={value} value={value}>
                {value}
              </SelectItem>
            ))}
          </SelectContent>
        </Select>
      </div>

      <div className="flex flex-col gap-1">
        <label className="text-[10px] font-bold uppercase text-card-beige-muted-foreground">
          Cliente
        </label>
        <Select value={filters.clientId} onValueChange={(v) => set("clientId", v ?? "all")}>
          <SelectTrigger className="w-[180px]" size="sm">
            <SelectValue placeholder="Todos" />
          </SelectTrigger>
          <SelectContent>
            <SelectItem value="all">Todos</SelectItem>
            {clients.map(([id, name]) => (
              <SelectItem key={id} value={id}>
                {name}
              </SelectItem>
            ))}
          </SelectContent>
        </Select>
      </div>

      <div className="flex flex-col gap-1">
        <label className="text-[10px] font-bold uppercase text-card-beige-muted-foreground">
          Status
        </label>
        <Select value={filters.status} onValueChange={(v) => set("status", (v as GoalStatus | "all") ?? "all")}>
          <SelectTrigger className="w-[150px]" size="sm">
            <SelectValue placeholder="Todos" />
          </SelectTrigger>
          <SelectContent>
            <SelectItem value="all">Todos</SelectItem>
            {(Object.keys(GOAL_STATUS_LABEL) as GoalStatus[]).map((status) => (
              <SelectItem key={status} value={status}>
                {GOAL_STATUS_LABEL[status]}
              </SelectItem>
            ))}
          </SelectContent>
        </Select>
      </div>

      {hasActiveGoalFilters(filters) ? (
        <Button
          type="button"
          size="sm"
          variant="ghost"
          onClick={() => onChange(DEFAULT_GOAL_FILTERS)}
          className="gap-1 text-xs"
        >
          <X className="h-3.5 w-3.5" />
          Limpar filtros
        </Button>
      ) : null}
    </div>
  );
}
